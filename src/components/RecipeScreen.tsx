...
const RecipeScreen: React.FC<RecipeScreenProps> = ({
	...
	const [isShowDietaryRestrictionsModal, setIsShowDietaryRestrictionsModal] =
	...
	const closeDietaryRestrictionsModalAndGetNewRecipe = useCallback(() => {
		setIsShowDietaryRestrictionsModal(false);
		getAIRecipe();
	}, [getAIRecipe]);
		...
		<div>
			...
			<div className="filterButtons">
				<AnimatedButton
					...
					onClick={() => setIsShowFridgeModal(true)}
				/>
				<AnimatedButton
					text={'Dietary Restrictions?'}
					onClick={() => setIsShowDietaryRestrictionsModal(true)}
				/>
			</div>
			<RecipeDisplay
				recipe={recipe.recipe}
				isOpen={isShowRecipe}
				onClose={() => setIsShowRecipe(false)}
			/>
			<WhatsInMyFridgeModal
				isOpen={isShowFridgeModal}
				onSave={closeFridgeModalAndGetNewRecipe}
				onClose={() => setIsShowFridgeModal(false)}
			/>
			<DietaryRestrictionsModal
				isOpen={isShowDietaryRestrictionsModal}
				onSave={closeDietaryRestrictionsModalAndGetNewRecipe}
				onClose={() => setIsShowDietaryRestrictionsModal(false)}
			/>
		</div>
	);
};
